-- Local regression test after operator applies migrations 18 and 19. All fixture changes roll back.
BEGIN;
...
UPDATE public.customer_activations SET user_id='ec180000-0000-4000-8000-000000000002',activated_at=now()
WHERE agreement_id=(SELECT id FROM public.customer_agreements WHERE application_id='ec180000-0000-4000-8000-000000000003');
UPDATE auth.users SET email='verified-new-address@qa.example',email_confirmed_at=now() WHERE id='ec180000-0000-4000-8000-000000000002';
SELECT lives_ok($$UPDATE public.customer_activations SET updated_at=now() WHERE user_id='ec180000-0000-4000-8000-000000000002'$$,'Verified account email change preserves activation identity');
SELECT throws_ok($$UPDATE public.customer_activations SET user_id='ec180000-0000-4000-8000-000000000001' WHERE user_id='ec180000-0000-4000-8000-000000000002'$$,'22023','immutable_activation_identity','Email change never permits account reassignment');
SELECT throws_ok($$UPDATE public.customer_activations SET activated_at=NULL WHERE user_id='ec180000-0000-4000-8000-000000000002'$$,'22023','immutable_activation_identity','Activated identity cannot be reset');
UPDATE auth.users SET email_confirmed_at=NULL WHERE id='ec180000-0000-4000-8000-000000000002';
SELECT throws_ok($$UPDATE public.customer_activations SET updated_at=now() WHERE user_id='ec180000-0000-4000-8000-000000000002'$$,'42501','verified_account_required','Activated account still requires verified Auth identity');
-- Restore verification and complete the original atomic onboarding RPC after email change.
UPDATE auth.users SET email_confirmed_at=now() WHERE id='ec180000-0000-4000-8000-000000000002';
SET LOCAL ROLE authenticated;
SELECT set_config('request.jwt.claim.sub','ec180000-0000-4000-8000-000000000002',true);
SELECT lives_ok($$SELECT public.create_onboarding_business(
 'activation-identity-qa','{"en":"Identity QA"}'::jsonb,NULL,'restaurant','en','Asia/Jerusalem','ILS',
 (SELECT id FROM public.plans WHERE code='pro'),NULL,NULL,NULL,NULL,NULL,NULL,NULL,
 'main','{"en":"Main"}'::jsonb,NULL,NULL,'QA street','QA city','IL','[]'::jsonb
)$$,'Verified UUID completes onboarding after email change');
SELECT is((SELECT role FROM memberships WHERE user_id=auth.uid() AND business_id=(SELECT id FROM businesses WHERE slug='activation-identity-qa')),'owner'::text,'Onboarding creates owner membership');
RESET ROLE;
SELECT is((SELECT agreed_amount_ils FROM customer_subscriptions WHERE business_id=(SELECT id FROM businesses WHERE slug='activation-identity-qa')),1821.40::numeric,'Subscription preserves approved snapshot despite changed catalog');
SELECT is((SELECT status FROM customer_subscriptions WHERE business_id=(SELECT id FROM businesses WHERE slug='activation-identity-qa')),'active'::text,'Onboarding activates subscription atomically');
SELECT ok((SELECT business_id IS NOT NULL AND onboarded_at IS NOT NULL FROM customer_activations WHERE user_id='ec180000-0000-4000-8000-000000000002'),'Business and activation linked atomically');
SELECT * FROM finish();
ROLLBACK;
