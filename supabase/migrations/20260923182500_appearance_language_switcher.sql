-- Authored only. Incremental migration: Support showLanguageSwitcher in save_restaurant_appearance.
...
DECLARE current_row public.restaurant_appearance;k text;tk text;new_revision integer;
BEGIN
 ...
 jsonb_typeof(p_settings->'density') IS DISTINCT FROM 'string' OR p_settings->>'density' NOT IN ('airy','balanced','compact') OR jsonb_typeof(p_settings->'images') IS DISTINCT FROM 'boolean'
 ...
 END IF;
 IF p_settings ? 'theme' THEN
 IF jsonb_typeof(p_settings->'theme') IS DISTINCT FROM 'object' THEN RAISE EXCEPTION 'invalid_settings' USING ERRCODE='22023'; END IF;
 IF (p_settings->'theme' - ARRAY['primary','accent','pageBackground','cardBackground','alternateSurface','text','textMuted','border','navBackground','activeCategoryBackground','activeCategoryText','buttonBackground','buttonText','priceBackground','priceText','heroOverlay','openStatus','closedStatus','modalBackground','modalText','footerBackground','footerText']) <> '{}'::jsonb THEN RAISE EXCEPTION 'invalid_settings' USING ERRCODE='22023'; END IF;
 FOR tk IN SELECT jsonb_object_keys(p_settings->'theme') LOOP
 IF jsonb_typeof(p_settings->'theme'->tk) IS DISTINCT FROM 'string' OR p_settings->'theme'->>tk !~ '^#[0-9a-fA-F]{6}$' THEN RAISE EXCEPTION 'invalid_settings' USING ERRCODE='22023'; END IF;
 END LOOP;
 END IF;
 IF p_settings ? 'showLanguageSwitcher' THEN
 IF jsonb_typeof(p_settings->'showLanguageSwitcher') IS DISTINCT FROM 'boolean' THEN RAISE EXCEPTION 'invalid_settings' USING ERRCODE='22023'; END IF;
 END IF;
